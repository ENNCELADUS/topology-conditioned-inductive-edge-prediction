...
- Use independent adversarial review agents only after completing a coding implementation wave; 
  if the task is simple and you are confident enough, don't spawn subagents.
...
GPU work runs only in the H20 container via `hpc/run.sh`; world size and score shards are
