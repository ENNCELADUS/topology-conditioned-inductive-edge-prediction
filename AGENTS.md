...
- KD supervision is training-only: training row/context banks contain no validation teacher targets. An optional separate G_val oracle bank scores the exact val_cls pairs for per-epoch KD diagnostics only; it never supplies training targets or selection losses. Student early stopping uses validation task BCE; checkpoint selection still uses V_val AUPRC and the five topology metrics. Oracle validation/test on their respective true graphs are separate ceiling diagnostics.
...
- `train_graph.pkl` (original train positives plus validation positives) is the `V_val` substrate. A single FIFO BFS from a seeded five-neighbor root stops before exceeding 20% of substrate positive pairs, counting self-loops once (cap 10,728; actual 10,719 positives / 1,250 nodes). Only V_val-internal pairs are quarantined; cross-boundary positive and negative pairs remain training samples over all 8,072 substrate nodes. Fixed FIFO BFS buckets use sizes 20–200, 50 draws each. V_val is pair-disjoint with training node exposure; test is node-disjoint. Old split-dependent checkpoints, structural caches, banks and results do not carry over; see `data/val_region/breadth_first.json`.
