...
- `train_graph.pkl` (original train positives plus validation positives) is the `V_val` substrate. A single FIFO BFS from a seeded five-neighbor root stops before exceeding 10% of substrate positive pairs, counting self-loops once (cap 5,364; actual 5,354 positives / 642 nodes; root `node_002696`, split seed 273). V_val is node-held-out (since 2026-09-08): its nodes leave the training universe and every pair touching them (internal and cross-boundary, positive and negative) is held out, so training covers 7,430 nodes and 38,916 positives, and dynamic negatives never touch V_val. Fixed FIFO BFS buckets use sizes 20–200, 50 draws each. Old split-dependent checkpoints, structural caches, banks and results do not carry over; see `data/val_region/breadth_first.json`. Root selection used test structure to minimize relative edge-count distribution mismatch; provenance is in `docs/results/validation_density_selection/README.md`.
