...
| ↳ `shuffle` (universe-level null) | — | 0.5789 | 0.5964 | 0.6412 | 0.265 | 1.090 | 25.6 | 41.9 | 5.7 | 2.38 |
...
| ↳ `shuffle` (universe-level null) | 0.5533 | 0.5567 | 0.388 | 0.410 | 0.500 | 0.667 | 0.013 | 0.289 | 0.559 | 9.8 | 27.3 | 9.5 |
...
1. **Outcome 1.** `topo_prompt_full` ≫ `prefix_base` on V_val in both families (val_cls AUPRC
   ...
   trained with the prompt present), and the universe-level `shuffle` drops it to near chance
   (val_cls AUROC 0.579, test 0.553; ball union 0.641, GS 0.265), far *below* the base:
   misleading structure is worse than none, so the reader relies on the coordinates rather
   than on their mere presence — almost to the exclusion of the endpoint attributes. The interface transmits
...
The scorer now draws one seeded permutation of the whole universe before sharding
(`src.score_universe._shuffle_source_rows`, commit cbb943a; artifacts record
`prefix_shuffle_scope: universe`); the same correction applies to the prefix arms' pair-condition
shuffle. Rerun on 30838 (17:34–17:46 UTC): val_cls AUROC/AUPRC 0.579/0.596 and test 0.553/0.557
(near chance), while the ball-union universe (positives 1.8 %, mixed within shards) moved only
