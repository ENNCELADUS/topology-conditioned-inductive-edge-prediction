...
scores test and test_topology and writes the combined edge+graph `test_report.json` before
...
scoring passes in one ledger epoch: fixed-0.5 test classification and per-subgraph
RD-matched sampled topology.
...
`scores/{test,test_topology}.npz` plus their shared `f0_cache_test_support.pt`/
`grounding_cache_test_support.npz` into whatever `--output-dir` it is given. It never writes a
