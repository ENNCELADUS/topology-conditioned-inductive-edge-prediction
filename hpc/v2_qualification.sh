#!/usr/bin/env bash
# Fail-closed launcher for the §13.19 v2 qualification and bound formal contexts.
set -euo pipefail
...
readonly FULL_CONFIG="configs/egostitch_e2e_stability_v2_breadth_first.yaml"
readonly QUALIFICATION_GPU_COUNT=2
readonly FORMAL_GPU_COUNT=4
readonly QUALIFICATION_GPU_IDS="0,1"
readonly FORMAL_GPU_IDS="0,1,2,3"
readonly EXPECTED_GPU_NAME="NVIDIA H20"

usage() {
  cat <<'EOF'
Usage:
  hpc/v2_qualification.sh qualify
  hpc/v2_qualification.sh formal <full|f_only|pair_topology|p0>

qualify is train/validation-only and requires exactly 2 visible NVIDIA H20s. It
runs sanity -> registered 2,000-step overfit -> full-arm rehearsal, stopping at
the first failure. The DRAFT registration is never edited or promoted.

formal requires exactly 4 visible NVIDIA H20s and a fully resolved BINDING v2
...
assert_qualification_boundary() {
  [[ "$(registration_status)" == "DRAFT" ]] || \
    fail "qualification requires the v2 registration to remain DRAFT"
...
assert_formal_registration() {
  [[ "$(registration_status)" == "BINDING" ]] || \
    fail "formal v2 training requires registration status BINDING"
  if registration_has_unresolved_marker; then
    fail "formal v2 training refuses unresolved REQUIRED-BEFORE-BINDING markers"
  fi
}
...
run_qualification() {
  assert_gpu_selection "${QUALIFICATION_GPU_COUNT}" "${QUALIFICATION_GPU_IDS}"
  assert_qualification_boundary
  REGISTRATION_SHA256_BEFORE="$(registration_sha256)"
  export REGISTRATION_SHA256_BEFORE
  trap assert_registration_unchanged EXIT

  echo "v2 qualification stage 1/3: sanity"
  "${UV_BIN}" run pytest -q \
    tests/test_train_egostitch.py \
    tests/test_e2_pipeline.py \
    tests/test_hpc_v2_qualification.py
  assert_registration_unchanged

  echo "v2 qualification stage 2/3: registered 2,000-step overfit"
  "${PYTHON_BIN}" -m src.e2_pipeline --config "${FULL_CONFIG}" \
    --worker-module src.train_egostitch \
    --v2-run-kind overfit \
    --output-dir outputs/egostitch_e2e_stage1_v2/qualification/overfit
  assert_registration_unchanged

  echo "v2 qualification stage 3/3: exact full-arm rehearsal"
  "${PYTHON_BIN}" -m src.e2_pipeline --config "${FULL_CONFIG}" \
    --worker-module src.train_egostitch \
    --v2-run-kind rehearsal \
    --output-dir outputs/egostitch_e2e_stage1_v2/qualification/rehearsal
  assert_registration_unchanged
  echo "v2 qualification completed; registration remains DRAFT"
}

formal_config() {
  case "$1" in
    full) echo "configs/egostitch_e2e_stability_v2_breadth_first.yaml" ;;
    f_only) echo "configs/egostitch_e2e_stability_v2_f_only_breadth_first.yaml" ;;
    pair_topology) echo "configs/egostitch_e2e_stability_v2_pair_topology_breadth_first.yaml" ;;
    p0) echo "configs/egostitch_e2e_stability_v2_p0_breadth_first.yaml" ;;
    *) fail "unknown formal arm: $1" ;;
  esac
}

assert_full_preflight() {
  local metadata="outputs/egostitch_e2e_stage1_v2/full/run_metadata.json"
  [[ -s "${metadata}" ]] || \
...
run_formal() {
  local arm="$1"
  local config
  assert_gpu_selection "${FORMAL_GPU_COUNT}" "${FORMAL_GPU_IDS}"
  assert_formal_registration
  if [[ "${arm}" != "full" ]]; then
    assert_full_preflight
  fi
