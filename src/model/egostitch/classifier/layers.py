"""Reusable building blocks for the classifier component (three-component refactor design §5).

Split off `src/model/B0.py`'s standalone V3.1 model (consolidation pass):
these classes and functions are genuinely reusable by any pairwise
classifier -- `SiameseEncoder`, `MLPHead`, `PairCrossAttention`, and their
supporting pooling/readout machinery -- as opposed to `b0_v31.py`, which
holds what is specific to the V3.1 baseline itself (its config-parsing
`__init__`, `V3_1`, and the `B0V31PairClassifier` that wraps these blocks
with a topo-conditioning socket).
"""
