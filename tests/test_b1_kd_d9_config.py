...
def test_kd_d9_config_builds_pair_latent_arm() -> None:
    ...
    assert config.output_dir == Path("outputs/b1_stage_v3/kd_d9")
    assert config.distill is not None
    assert config.distill.arm == "kd_d9"
    assert config.distill.targets_path == "outputs/distill/kd_targets_breadth_first_seed0_v4"
    ...
    assert config.distill.anchors_per_step == 2
