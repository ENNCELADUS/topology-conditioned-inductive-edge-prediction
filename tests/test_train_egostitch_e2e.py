...
class TestPrepareAndAssembleE2E:
    ...
    def test_assembly_builds_a_fixed_smaller_cascade_topology_panel(
        ...
        cfg = replace(
            ...
            topology_validation=te.EgoTopologyValidationConfig(
                ...
                cascade_complement_sample_size=2,
            ),
        )
        ...
        assert all(
            len(balls) == 2
            for balls in data.val_cascade_topology_reference.bucket_ref.node_sets.values()
        )
        assert data.val_cascade_ball_union.sample_u_idx.size == min(
            2, data.val_cascade_ball_union.complement_total
        )
    ...
    def test_cascade_validation_scores_only_the_cascade_panel(
        ...
        assert result.fidelity["topology_scored_rows"] == (
            data.val_cascade_ball_union.u_idx.size + data.val_cascade_ball_union.sample_u_idx.size
        )
