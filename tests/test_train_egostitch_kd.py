...
from src.train_egostitch import EgoDistillConfig, _build_kd_runtime, _CompositeStep

pytestmark = pytest.mark.unit


def _tiny_mapping(generator_name: str, *, encoder_dim: int = 16) -> dict[str, object]:
    return {
...
def test_kd_terms_are_finite_and_backpropagate_into_the_student_encoder(
    ...
    node_ids = [str(node) for node in range(5)]
    generator = cast(FullEgoFeaturesGenerator, student.generator)
    generator.set_oracle_context(truth, node_ids)
    generator.set_node_features(torch.randn(5, student.input_dim), node_ids)
    ...
    seed, gram = step._kd_terms(graph, embedding_ab, embedding_ba, torch.ones(2))
    ...
    seed_masked, gram_masked = step._kd_terms(
